...
export function Block({ content, context }: FileBlockProps) {
  ...
  const onSubmit = async (annotations: AnnotationType[], title: string, description: string) => {
    const url = `https://github-blocks.vercel.app?owner=${owner}&repo=${repo}&path=${path}&viewerOverride=annotate-react&annotations=${encodeURIComponent(JSON.stringify(annotations))}&componentDefinition=${encodeURIComponent(JSON.stringify(componentDefinition))}`
